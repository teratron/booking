{{--
    ...
    inactive or invented `{lang}` segment) — never indexable.
--}}
<x-layouts.public :title="__('public.legal.not_found.title')" :noindex="true">
    <div class="mx-auto max-w-2xl px-4 py-24 text-center">
        <p class="text-sm font-semibold text-brand">404</p>
        <h1 class="mt-2 text-3xl font-semibold text-ink">{{ __('public.legal.not_found.title') }}</h1>
        <p class="mt-4 text-ink-muted">{{ __('public.legal.not_found.body') }}</p>
