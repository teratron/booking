...
return [
    ...
    */
    ...
    |
    | Deliberately "redis", never the package's own "storage" default: the
    | "storage" driver writes every recorded entry straight to the
    | `pulse_entries` table inside the same request/response cycle a
    | recorder observed (Pulse::ingest() runs from a terminating callback,
    | which Laravel's own testing harness executes before a Feature test
    | inspects its query log) — on the public territory page, tuned to
    | exactly 30 queries with zero headroom (see
    | tests/Feature/Public/PublicPerformanceBudgetTest.php), that extra
    | write is the difference between passing and failing. "redis" instead
    | pushes every entry onto a Redis stream with no SQL involved; a
    | separate `pulse:work` process (see docker-compose.yml) drains that
    | stream into storage on its own schedule, off the request path
    | entirely — the same request/ingest decoupling Laravel's own docs
    | recommend for any traffic volume above trivial.
    |
    */
