...
return [
    ...
    |--------------------------------------------------------------------------
    |
    | The language registry is data, not code — this array exists only
    | because the package's own Locales helper throws when given an empty
    | one. `en` is the minimal non-empty bootstrap value; every real locale
    | (active or not) is registered from the `languages` table at boot time
    | (see AppServiceProvider), which is the actual source of truth and is
