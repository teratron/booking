...
| Branches | **`master` only** — the sole head on the remote, and the working branch. `develop` was removed and no protection rule remains on `master`: `branches/master/protection` returns `404 Branch not protected`, and `rulesets` returns `[]`. Deliberate, not drift — the project's own engineering-conventions document and branch-model runbook record the pause and its resumption conditions, and the prior working state (both branches protected, `develop` present) is preserved at a named git tag for exact restoration. This row previously described that prior state and was accurate when written. |
...
**This table is the target state — not the model running today.** Per
[l1-release-operations.md](l1-release-operations.md) §3.3's interim clause, the
repository runs a single line, `master`, worked on directly, until the product is
launched in production at the client or a second developer joins, whichever comes first.
Nothing below is weakened or reinterpreted for the interim; the transitions the rules
govern simply do not occur while there is one line and nothing deployed. The interim
state has exactly two rules of its own, and they are the whole of it:
...
Everything below activates together at resumption, with the protection settings §5.11
...
> **Currently paused, deliberately — state, not design.** Under §5.2's interim
> single-line model there is no protection rule on `master`, so the protection settings
> below are absent and the ownership file is consulted by nothing. Only the tree-walking
> test still runs, and a test whose result no merge path reads is documentation. This was
> taken down in the same recorded decision that opened the interim state, not lost to
> drift — but the resulting gap is identical either way, which is why it is stated as
> plainly as if it were. Nothing in this section is withdrawn or rewritten for the
> interim — the mechanism described here is the mechanism to restore, and restoring it is
> a precondition of the first release, not a follow-up to it. The settings table below
> doubles as the restoration procedure, including its ordering constraint. Verify by
> reading the live state back, never by assuming the paused configuration matches what
> was recorded when it was paused — a decision to restore it can itself go stale.
