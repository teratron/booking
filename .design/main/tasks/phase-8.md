...
- **Notes:** A GitHub App installation — **not** a personal access token and not a shared account — so its actions are attributable to it rather than to whoever created its credential. The withheld column is the whole point: it may open, update and merge pull requests, push tags, comment and report, but it cannot approve reviews on `master`, cannot change its own permissions, and holds no production-tier credential. An identity that can both request and grant its own promotion has no gate at all. The permission set granted (Contents, Pull requests, Issues: read/write; Metadata, Checks: read-only; Administration, Secrets, Environments, Actions: none) is phrased generically enough to hold regardless of which branch it opens PRs against — it did not need rewriting for the 2026-08-22 branch-model reconciliation. **C12 quarantine is a separate axis and this closing does not clear it**: this task's own blocker (no browser-free path to create a GitHub App) was never the same thing C12 tracks (the L1 parent's `RFC` status), and completing the former has no bearing on the latter — see `T-8E03`/`T-8T03`, still `Blocked [!] (C12)`, and [l1-release-operations.md](../specifications/l1-release-operations.md) §3.3 for what actually lifts it.
- **Changes:** GitHub App created and installed on `teratron/booking` via the browser (App ID `4689159` — no API path exists for this step, confirmed absent in both `gh api` and the REST docs). Permission grant confirmed by the owner against the table above, read directly from the app's own Settings → Permissions page, since verifying it by API requires a JWT signed with the app's private key — an authentication path deliberately not available to this agent, matching the whole point of the withheld column. Live-checked independently, with a plain user token: the app does not appear in `production`'s reviewer list (`teratron` only) or in `.github/CODEOWNERS` (`@teratron` only) — both empty of it, as `T-8E02`'s own handoff requires. `AUTOMATION_APP_ID` recorded in the local `.env` (gitignored, confirmed via `git check-ignore`) pending `T-8E03`'s repository-secret tier; the private key never left the owner's machine and was not seen by the agent.
