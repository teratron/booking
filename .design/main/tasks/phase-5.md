...
- **Status:** Todo
- **Assignment:** Agent
- **Verify:** `docker compose exec app ./vendor/bin/pest --filter=CatalogQueryService` proves the ordering contract exactly: tier rank ascending, then within a tier pinned position, most recent bump, active promotion weight, created date, rating — proven with a fixture deliberately constructed so every criterion after the first would produce a different order if any earlier one were skipped, falsifying tier-first ordering directly; scope resolution expands a territory to itself plus every descendant; filters (amenities, price, rating, distance, catering) combine with AND semantics and round-trip through the same parameter shape home/catalog/territory pages will share; a type's own `attribute_schema` and applicable amenity groups gate which fields and facets render, proven against two differently-typed fixtures; pagination is offset-addressable, not infinite-scroll-only.
...
- **Notes:** This is the phase's single highest-leverage decision per the spec's own Implementation Note #1 — home, territory, catalog, and search pages must all resolve through this one contract. Build the indexes the ordering implies (country, region, city, resort, type, package, publication status, moderation status, bump date, package expiry, name, slug, publication date, language) with this task, not after a slowdown (Implementation Note #2). Cache per (scope, filters, sort, page, language, country); invalidation keys are read directly from the existing bump/package-change/moderation-approval/availability-toggle/object-edit writes Phases 2–4 already produce, never a second invalidation scheme (Implementation Note #3).
