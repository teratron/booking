...
- **Task:** `T-6A02` (redirect table) done — 2/16. Redirect model, administration resource, `ResolveRedirect` middleware, and same-operation creation wired into every slug-change path (territory edit, territory reparent, object edit direct and moderated). Track A is now fully done. Next: `T-6C01`/`T-6D01` (independent starting points) or Track B (waits on Track A, now clear).
...
- **Next Action:** Execute T-6B01 Per-language SEO metadata fields and the resolution ladder via /magic.run main
...
Implementation: [21/21] Phase 1 DONE · [25/25] Phase 2 DONE · [23/23] Phase 3 DONE · [16/16] Phase 4 DONE · [18/18] Phase 5 DONE · [2/16] Phase 6 ACTIVE
```
...
- 2026-08-15 **Decision: `T-6A01` (URL grammar) closed** — new `SeoUrlGrammarTest.php` (8 cases) plus real product changes: two new services (`PublicUrlGenerator`, `PublicSlugResolver`), two migrations (slug on `object_type_translations`; `country_id` denormalized onto `territory_translations` so its uniqueness scopes to `(locale, country_id, full_slug_path)` — a bare path-only constraint would reject two identically-named root territories in different countries, which the spec explicitly allows), and a genuine admin-panel bug fix (the `ObjectType` Filament form had no slug field at all). **`composer test:coverage` fails project-wide at 78.9% (floor 80%)** — traced to pre-existing, untouched Phase 3 gaps (`PromotionLifecycleService` 0%, `NewsItemLifecycleService` 35.9%), not new code; flagged, not silently absorbed.
- 2026-08-15 **Decision: `T-6A02` (redirect table) closed — Track A complete** — new `RedirectResolutionTest.php` (6 cases) plus real product changes: `Redirect` model/policy/Filament resource, `RedirectRegistrar` (the sole write path, collapses chains), `ResolveRedirect` middleware, wired into territory edits, reparenting, and both object slug-edit paths (direct admin, moderated). Two real bugs found only by running the actual HTTP flow: `Illuminate\Routing\Pipeline` renders a thrown 404 into a Response *inside* `$next()`, before any outer middleware `catch` block can see it — a try/catch around `$next($request)` never fires; fixed by inspecting `$response->getStatusCode()` instead. `TerritoryAdministrator::recomputeSlugPaths()` was not itself transaction-wrapped (only its `reparent()` caller was) — a refused reuse partway through a subtree could have left earlier nodes rewritten; wrapped now. `PanelAuthorizationMatrixTest`'s own exhaustive axis-less-resource enumeration needed updating (by design — it throws if a new global resource has no probe fixture). Full non-slow suite: 674 passed (up from 668), 0 failed, 3 skipped. `composer analyse`/`pint --test`/architecture suite clean.
